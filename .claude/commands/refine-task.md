...
/refine-task .specs/tasks/draft/add-validation.feature.md

# Fast refinement with minimal stages
/refine-task .specs/tasks/draft/quick-fix.bug.md --fast

# Continue from a specific stage
/refine-task .specs/tasks/draft/complex-feature.feature.md --continue decomposition

# High-quality refinement with checkpoints
/refine-task .specs/tasks/draft/critical-api.feature.md --target-quality 4.5 --human-in-the-loop 2,3,4,5,6
```
...
1. **Validate task file exists:**
   - Check that `TASK_FILE` exists in `.specs/tasks/draft/`
...
6. **Ensure directories exist**:
   ...
   ```bash
   mkdir -p .specs/tasks/todo .specs/tasks/in-progress .specs/tasks/done .specs/research .specs/analysis .specs/scratchpad
   ```
...
- **Task file must exist in `.specs/tasks/draft/` before running this command!**
...
- **Prompt**:
  ...
  CRITICAL: DO NOT OUTPUT YOUR RESEARCH, ONLY CREATE THE SCRATCHPAD AND RESEARCH FILE.
  ```
...
1. Use git tool to stage the task file, research file, analysis file, and scratchpad files (only those that were created)
...
| **Research** | `<research file path or "Skipped">` |
...
├── research/
│   └── research-<name>.md       # Research document (if research stage ran)
├── analysis/
...
1. Review task: `.specs/tasks/todo/<filename>`
   ...
   - Run `/refine-task` again with `--continue` to incorporate your feedback — it detects changes against git and propagates updates **top-to-bottom** (editing a section only affects sections below it, not above)
2. If everything is fine, begin implementation: `/implement-task` (will auto-select the task from todo/)
```
